...
export async function GET(request:Request){
  try{
    ...
    const [time,finance,reserves,next,settings,mail,ecgPlans]=await Promise.all([
      ...
      sql`SELECT external_id,title,location,issue_priority,issue_status,estimated_minutes,planning_priority,notes,blocked,updated_at FROM org_ecg_issue_plans WHERE owner_id=${ownerId} AND issue_status<>'done' ORDER BY blocked,CASE planning_priority WHEN 'dringend' THEN 0 WHEN 'diese_woche' THEN 1 ELSE 2 END,updated_at DESC`
    ]);
    ...
    return Response.json({
      ...
      reserve_monthly_cents:Number((reserves[0] as any).monthly_required_cents),next,mail,ecg_plans:ecgPlans,ecg_planned_minutes:ecgPlans.reduce((sum:any,p:any)=>sum+Number(p.estimated_minutes||0),0),ecg_unestimated_count:ecgPlans.filter((p:any)=>p.estimated_minutes==null).length
    });
  }catch(error){
