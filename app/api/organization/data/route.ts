...
export async function POST(request:Request){
  try{
    ...
    if(!validEntities.has(entity))return Response.json({error:"Unbekannter Datenbereich."},{status:400});
    const sql=getSql();let rows:any[]=[];
    if(entity==="time")rows=await sql`INSERT INTO org_time_entries(owner_id,source,area,activity,customer,location,notes,started_at,ended_at,travel_minutes,billable,volunteer,sunday,external_id) VALUES(${ownerId},${data.source||"organization"},${data.area||"erhard"},${data.activity||"Allgemeine Tätigkeit"},${data.customer||null},${data.location||null},${data.notes||null},${data.started_at},${data.ended_at||null},${Number(data.travel_minutes)||0},${!!data.billable},${!!data.volunteer},${!!data.sunday},${data.external_id||null}) RETURNING *`;
...
export async function PATCH(request:Request){
  try{
    ...
    const sql=getSql();let rows:any[]=[];
    if(entity==="time")rows=await sql`UPDATE org_time_entries SET area=${data.area},activity=${data.activity},customer=${data.customer||null},location=${data.location||null},notes=${data.notes||null},started_at=${data.started_at},ended_at=${data.ended_at||null},travel_minutes=${Number(data.travel_minutes)||0},billable=${!!data.billable},volunteer=${!!data.volunteer},sunday=${!!data.sunday},updated_at=now() WHERE id=${id} AND owner_id=${ownerId} RETURNING *`;
