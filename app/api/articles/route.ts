...
function norm(s:string){return s.toLowerCase().replace(/ß/g,"ss").normalize("NFD").replace(/[\u0300-\u036f]/g,"")}
function score(a:Article,q:string){
 if(!q)return 1;
 const words=norm(q).split(/[\s,;]+/).filter(Boolean).map(w=>w==="trockner"?"trockn":w);
...
export async function GET(r:Request){
 ...
 const requested=Number(u.searchParams.get("limit")||40),limit=Number.isFinite(requested)?Math.max(1,Math.min(Math.floor(requested),100)):40;
 try{
  const sql=getSql();
  ...
  if(dbRows.length){
   const rows=dbRows.map((x:any)=>({catalog:x.category||"own",article_number:x.article_number||"",name:x.name,description:x.description||x.name,unit:x.unit||"",unit_price:x.unit_price==null?null:Number(x.unit_price),source:x.source||"Excel-Import"}));
