...
export async function POST(_r:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const ex=await sql`SELECT id FROM assignments WHERE case_id=${id} ORDER BY created_at DESC LIMIT 1`;
  if(ex.length){
   ...
   return Response.json({id:ex[0].id,offer_id:o[0].id})
  }
  const a=await sql`INSERT INTO assignments(case_id,company_id,assignment_date,scope,insurer,claim_number,insurance_number)
    VALUES(${id},${c[0].company_id},CURRENT_DATE,${scope},${c[0].insurer},${c[0].claim_number},${c[0].insurance_number}) RETURNING id`;
  return Response.json({id:a[0].id,offer_id:o[0].id},{status:201})
