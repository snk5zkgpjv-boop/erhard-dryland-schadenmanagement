...
export async function POST(r:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const rows=await sql`
      ...
      VALUES(${id},${c[0].company_id},COALESCE(${txt(b.report_date)}::date,CURRENT_DATE),${txt(b.report_number)},${txt(b.worker_name)},${txt(b.function)},${txt(b.work_start)}::time,${txt(b.work_end)}::time,${intv(b.break_minutes)},${intv(b.vehicle_count)},${intv(b.trips_count)},${txt(b.description)},${txt(b.follow_up_work)})
      RETURNING id`;
