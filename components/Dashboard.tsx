...
export default function Dashboard(){
 ...
 return <>
  <section className="hero"><span className="pill">Fachmodul</span><h1>Schadenmanagement</h1><p className="muted">Schadenakten, Angebote und Rechnungen für Erhard Dienstleistungen und Dryland Trocknungstechnik.</p>{auth?.user&&<p className="small muted">Angemeldet als <strong>{auth.user.display_name}</strong> · {auth.user.role==="admin"?"Admin":auth.user.role==="buero"?"Büro":"Techniker"}</p>}
   <div className="actions" style={{marginTop:12}}>{auth?.user?.role==="admin"&&<Link className="button success" href="/organization">Organisationszentrale</Link>}<Link className="button" href="/cases/new">+ Neuer Schaden</Link>{(auth?.user?.role==="admin"||auth?.user?.role==="buero")&&<><Link className="button secondary" href="/direct/new">Angebot/Rechnung direkt</Link><Link className="button secondary" href="/documents">Dokumente</Link><Link className="button secondary" href="/articles">Artikelliste</Link></>}{auth?.user?.role==="admin"&&<><Link className="button secondary" href="/equipment">Geräte</Link><Link className="button secondary" href="/settings/companies">Firmendaten</Link><Link className="button secondary" href="/users">Benutzer</Link></>}<button className="button secondary" onClick={load}>Aktualisieren</button><LogoutButton/></div>
  </section>
