...
export async function middleware(req:NextRequest){
  ...
  try{
    ...
    const rows=await sql`SELECT u.id,u.username,u.display_name,u.role FROM app_sessions s JOIN app_users u ON u.id=s.user_id WHERE s.token_hash=${hash} AND s.expires_at>now() AND u.active=true LIMIT 1`;
    if(!rows.length)throw new Error("session");
    const h=new Headers(req.headers);
    h.set("x-app-user-id",String(rows[0].id));h.set("x-app-username",String(rows[0].username));h.set("x-app-display-name",String(rows[0].display_name));h.set("x-app-role",String(rows[0].role));
